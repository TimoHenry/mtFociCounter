...
AUTHOR: Timo Rey; EPFL, 2020; MRC-MBU & EMBO fellow, 2021-23
LICENCE:  MIT licence

PURPOSE:
This is a macro intended to allow fully reproducible quantification of foci inside mitochondria
from fluorescence images of (mammalian) cell culture.
...
TODOs = newArray("Convert to Tiff", "Find Cells", "Find Foci");						// define list of options
...
Dialog.addMessage("3D image-stack is expected.")
Dialog.addCheckbox("Data is already 2D", false);									// default is false -> expect image stack (3D)
// Ask user which analysis they would like to do:
Dialog.addMessage("Default z-projection is Maximum Intensity over all images in a stack.")
Dialog.addMessage("You may adapt the Z-projection to: \n- change z-projection style \n- change z-projection range.")
Dialog.addCheckbox("Adapt Z-projection?", false);									// default is false -> max-projection of all z-steps
Dialog.addMessage("Click OK or press ENTER.")
Dialog.show();																		// generte the GUI

DIM_2				= Dialog.getCheckbox();											// get answer to whether input-data is already 2-dimensional
USER_Z 			 	= Dialog.getCheckbox();											// get answer to whether user wants to adapt z-projection
CELL_OUTLINE 		= true;															// true -> manual segmentation of cells
...
Dialog.addCheckbox("Subtract nucleus?", false);										// area will be subtracted from foci-channel (- filter)
Dialog.addNumber("If yes, which channel is this?", 3);
Dialog.addCheckbox("Filter with mitochondria?", false);								// area will be multiplied with foci-channel (+ filter)
Dialog.addNumber("If yes, which channel is this?", 2);
Dialog.addCheckbox("Localise foci:", true);
Dialog.addNumber("Which channel is this?", 1);
Dialog.addNumber("Start of prominence range", 5);									// ask user to decide on prominence
Dialog.addNumber("End of prominence range", 200);									// ask user to decide on prominence


Dialog.addMessage("How would you like to do the segmentation?");
seg_Options 	= newArray("automated", "manual", "pre-segmented");	
Dialog.addRadioButtonGroup("Nuclei:",seg_Options,2,1,"automated");						// default will segment nuclei manually.
Dialog.addRadioButtonGroup("Mitochondria:",seg_Options,2,1,"automated");				// default will segment mitochondria automatically.
...
with_foci				= newArray(Dialog.getCheckbox());							// whether foci should be detected
...
for (i=0; i<to_analyse.length; i++){
	...
	// split channels:
	if (num_channels > 1){															// check if more than 1 channels are available
		run("Split Channels");};													// if so, split image-stack
	else{rename("C1-"+to_analyse[i]);};												// if only 1 colour image is available, adapt its name

// option 1) if with nucleus:
	if(with_nucleus[0]){
		Nuc_Window = "C"+with_nucleus[1]+"-"+to_analyse[i];							// find correct image from stack
	// segment nucleus:
		selectWindow(Nuc_Window);

		if(with_nucleus[1]==with_mito[1] || with_nucleus[1]==with_foci[1]){			// check if user wants to double-use other channel
			run("Duplicate...", " ");												// create a duplicate
			Nuc_Window		= getTitle();};											// select the duplicate for nuc-procesing
...
// Find & filter foci:
	if(with_foci[0]){
		...
		else{findFoci(with_foci[2], with_foci[3], to_analyse[i],"1st");	// pass input variables for: (prom_start, prom_stop, foci_Window, foci_type)
		};
	};
...
function zProject(dir, fov, Z_STYLE){
	...
	if(DIM_2){rename("MAX_"+fov);};														// check if already 2D-image input.
	else{																				// otherwise, create z-projection
		if(Z_STYLE[1]){																	// with default values
			run("Z Project...", Z_STYLE[0]);};											// z-project
...
function segmentNucleus(segNuc_Style){
		...
		if(segNuc_Style=="manual"){
			...
			close(Nuc_Window);
			selectWindow("Mask");
			rename(Nuc_Window);};													// fill holes within nucleus		
		// external binary:
		if(segNuc_Style=="pre-segmented"){
