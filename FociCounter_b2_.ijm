...
// 1) get mitomasks (process channel 1):	
	...
	// Measure cell area:
	run("Set Measurements...", "area redirect=None decimal=1");			// Set measurement parameters to determine cell area
	run("Measure");														// Measure
	cell_area = getResult("Area", 0);									// Create variable to hold value
	close("Results");													// close
	...
	strt_stp = cell_depth(mito_files[i]);                           	// returns array with user-defined starting & stopping slice
																		// NOTE: Max-Intensity might be better for mito-signal.
...
// 3) filter foci:		
		...
		prominences = newArray(10000, 20000, 30000, 40000, 50000);		// could change range here
...
function quantifyFoci(name_remaining_foci, prominence){
	...
	File.append("../"+mito_files[i]+","+cell_area+","+start_slice+","+stop_slice+","+prominence+","+nROIs, OUT_DIR2+"Results_"+DATE+".csv");
